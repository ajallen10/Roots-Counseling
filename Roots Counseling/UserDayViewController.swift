...
class UserDayViewController: UIViewController {
    var week = ""
    var user = ""
    var dayNum = ""
    var isRoots = ""
...
//        print("Day \(dayNum): \(t2 - t1)")
    }

    func read(){
        let db = Firestore.firestore()
        let uid: String = Auth.auth().currentUser!.uid
        db.collection("users").document(uid).collection("Weeks").document(week).collection(dayNum).getDocuments() { (querySnapshot, err) in
            if let err = err {
